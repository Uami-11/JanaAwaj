<script>
  ...
  onMount(() => {
    ...
    // Load Nepalify for Romanized Nepali typing (reliable library)
    const script = document.createElement('script');
    script.src = 'https://unpkg.com/nepalify@1.1.0/dist/nepalify.min.js';
    script.async = true;
    script.onload = () => {
      // Enable Romanized typing only when lang = 'ne'
  ...
  // Enable Nepali Romanized typing on all inputs/textareas
  function enableNepaliTyping() {
    Nepalify.romanize(); // Set to Romanized layout
    // Apply to all form elements
    const inputs = document.querySelectorAll('input[type="text"], input[type="email"], textarea');
    inputs.forEach(input => {
      Nepalify.add(input); // Attach to each input
    });
  }
  ...
  function disableNepaliTyping() {
    const inputs = document.querySelectorAll('input[type="text"], input[type="email"], textarea');
    inputs.forEach(input => {
      Nepalify.remove(input); // Detach from inputs
    });
  }
  ...
  function goBack() { selectedCategory = null; subject = email = message = ''; files = []; captchaInput = ''; }

  function isFileAllowed(file) {
    const blocked = ['.exe','.bat','.sh','.js','.vbs','.ps1','.jar','.apk'];
    const ext = '.' + file.name.toLowerCase().split('.').pop();
    if (blocked.includes(ext)) { alert(currentT.fileBlocked); return false; }
    const allowed = ['image/jpeg','image/png','image/gif','image/webp','video/mp4','video/webm'];
    if (!allowed.includes(file.type)) { alert(currentT.fileInvalid); return false; }
    if (file.size > 8*1024*1024) { alert(currentT.fileLarge); return false; }
    return true;
  }

  function handleFileChange(e) {
    const newFiles = Array.from(e.target.files).filter(isFileAllowed);
    if (files.length + newFiles.length > 6) { alert(currentT.maxAttachments); return; }
    files = [...files, ...newFiles];
  }

  function removeFile(i) { files = files.filter((_, idx) => idx !== i); }

  function validateAndSubmit(e) {
    if (!subject || !message || captchaInput !== captchaNumber) {
      alert(currentT.fillAll);
      e.preventDefault();
      return;
    }
    ...
    // Form submits to Formsubmit for email (add action if needed)
  }
</script>
...
<main class="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
  ...
  <div class="max-w-4xl mx-auto px-6 py-12">
    {#if !selectedCategory}
      <div class="text-center mb-12">
        <h2 class="text-4xl font-bold text-gray-800 dark:text-white mb-4">{currentT.prompt}</h2>
        <p class="text-lg text-gray-600 dark:text-gray-300">{currentT.choose}</p>
      </div>
    ...
    {:else}
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 md:p-12">
        ...
        <h2 class="text-3xl font-bold text-gray-800 dark:text-white mb-3">
          {currentT.report} {lang === 'ne' ? selectedCategory.name_ne : selectedCategory.name}
        </h2>
        <p class="text-gray-600 dark:text-gray-300 mb-10">{currentT.reviewed}</p>
        <form on:submit|preventDefault={validateAndSubmit} class="space-y-8">
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{currentT.email}</label>
            <input bind:value={email} type="email" placeholder="you@example.com"
              class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{currentT.subject} <span class="text-red-500">*</span></label>
            <input bind:value={subject} type="text" required placeholder={lang === 'ne' ? "उदाहरण: बानेश्वरमा ट्राफिक लाइट बिग्रिएको" : "e.g. Broken traffic light at Baneshwor"}
              class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{currentT.description} <span class="text-red-500">*</span></label>
            <textarea bind:value={message} required rows="7" placeholder={lang === 'ne' ? "समस्याको विस्तृत विवरण लेख्नुहोस्... स्थान, समय, के भयो, आदि।" : "Describe the issue in detail... Location, time, what happened, etc."}
              class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent resize-none"></textarea>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{currentT.attachments}</label>
            <input type="file" accept="image/*,video/*" multiple on:change={handleFileChange}
              class="block w-full text-sm text-gray-500 file:mr-4 file:py-3 file:px-6 file:rounded-lg file:border-0 file:bg-primary file:text-white hover:file:bg-red-700" />
            {#if files.length > 0}
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{currentT.spam} <span class="text-red-500">*</span></label>
            <div class="flex items-center gap-6">
              <div class="bg-gray-800 text-white px-6 py-4 rounded-lg text-2xl font-mono tracking-wider">{captchaNumber}</div>
              <input bind:value={captchaInput} type="text" maxlength="4" required placeholder="????"
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">{currentT.spamHint}</p>
          </div>
          <div class="flex justify-end gap-4 pt-6">
            ...
            <button type="submit" disabled={!subject || !message || captchaInput !== captchaNumber}
              class="px-8 py-3 bg-primary text-white rounded-lg font-medium hover:bg-red-700 disabled:opacity-50 transition">
              {currentT.submit}
            </button>
          </div>
        </form>
  ...
  <!-- Bottom-Left Language Switch + Keyboard Layout Image -->
  <div class="fixed bottom-4 left-4 z-50 flex items-center gap-3 bg-white dark:bg-gray-800 p-3 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
    ...
    <a href="https://applyvr.election.gov.np/Image/nepali-unicode-romanized-v1.4-main-window-outlook.jpg" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 text-primary hover:underline">
      <img src="https://applyvr.election.gov.np/Image/nepali-unicode-romanized-v1.4-main-window-outlook.jpg" alt="Nepali Keyboard Layout" class="w-8 h-8 rounded border">
      Layout
    </a>
  </div>
</main>
